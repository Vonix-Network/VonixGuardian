...
- Every Guardian lane uses the same embedded release version: **`2.0.0`**.
...
- Pushing `v2.0.0` runs all nine cells and publishes the GitHub release only after the matrix succeeds.
...
- Preserve the loader-specific metadata and exact `2.0.0` expansion.
...
- [ ] Root AGENTS, README, docs, and changelog agree on Guardian `2.0.0`.
- [ ] All five version directories remain in this repository.
- [ ] All nine loader cells embed `2.0.0`.
- [ ] Manual CI dispatch cannot publish a release.
- [ ] CI build-only matrix passes 9/9.
- [ ] `v2.0.0` release assets and hashes read back remotely.
