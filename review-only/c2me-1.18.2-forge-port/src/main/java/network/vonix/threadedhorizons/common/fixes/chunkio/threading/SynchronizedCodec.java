package network.vonix.threadedhorizons.common.fixes.chunkio.threading;
...
public class SynchronizedCodec<A> implements Codec<A> {
    ...
    public <T> DataResult<Pair<A, T>> decode(DynamicOps<T> ops, T input) {
        try {
            ...
            return DataResult.error("Interrupted while decoding");
        } finally {
    ...
    public <T> DataResult<T> encode(A input, DynamicOps<T> ops, T prefix) {
        try {
            ...
            Thread.currentThread().interrupt();
            return DataResult.error("Interrupted while encoding");
        } finally {
            if (lock.isHeldByCurrentThread()) lock.unlock();
        }
    }
