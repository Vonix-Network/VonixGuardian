package network.vonix.threadedhorizons.common.util;

import com.google.common.base.Preconditions;
import net.minecraft.core.Holder;
import net.minecraft.world.level.biome.Biome;
import net.minecraft.world.level.chunk.PalettedContainer;

public class PalettedContainerUtil {

    private PalettedContainerUtil() {
    }

    public static Holder<Biome>[][][] toArray(PalettedContainer<Holder<Biome>> palettedContainer, int xSize, int ySize, int zSize) {
        if (palettedContainer == null) return null;
        final Holder<Biome>[][][] biomes = new Holder[xSize][ySize][zSize];
        for (int x = 0; x < xSize; x++) {
            for (int y = 0; y < ySize; y++) {
                for (int z = 0; z < zSize; z++) {
                    biomes[x][y][z] = palettedContainer.get(x, y, z);
                }
            }
        }
        return biomes;
    }

    public static void writeArray(PalettedContainer<Holder<Biome>> palettedContainer, Holder<Biome>[][][] array) {
        Preconditions.checkNotNull(palettedContainer);
        Preconditions.checkNotNull(array);
        palettedContainer.acquire();
        try {
            for (int x = 0, arrayLength = array.length; x < arrayLength; x++) {
                Holder<Biome>[][] biomes1 = array[x];
                for (int y = 0, biomesLength = biomes1.length; y < biomesLength; y++) {
                    Holder<Biome>[] biomes2 = biomes1[y];
                    for (int z = 0, biomeLength = biomes2.length; z < biomeLength; z++) {
                        Holder<Biome> biome = biomes2[z];
                        palettedContainer.set(x, y, z, biome);
                    }
                }
            }
        } finally {
            palettedContainer.release();
        }
    }

}
