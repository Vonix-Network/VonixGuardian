...
public final class NeoForgeBootstrap {
    ...
    /**
     * NeoForge's mod classloader does not reliably trigger JDBC service
     * discovery for Jar-in-Jar drivers. Explicitly initialize the selected
     * driver before the core DAO asks DriverManager for a connection.
     */
    private static void ensureDatabaseDriver(GuardianConfig config) throws SQLException {
        String driver = switch (config.database().type()) {
        ...
        try {
            Class.forName(driver, true, NeoForgeBootstrap.class.getClassLoader());
        } catch (ClassNotFoundException ex) {
            throw new SQLException("Configured " + config.database().type()
                + " JDBC driver is unavailable to the NeoForge mod classloader: " + driver, ex);
        }
    }
